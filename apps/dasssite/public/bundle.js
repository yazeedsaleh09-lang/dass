...
function rect(x, y, w, h2, fill, extra = "") {
  return `<rect x="${x}" y="${y}" width="${w}" height="${h2}" fill="${fill}"${extra ? " " + extra : ""}/>`;
}
...
function IncompleteScene() {
  const win = [
    { x: 78, y: 104, w: 148, h: 286 },
    // vertical → person A
    { x: 150, y: 398, w: 402, h: 96 },
    // horizontal → the table + decision
    { x: 452, y: 208, w: 150, h: 158 }
    // small → person B + red clue
  ];
  return `<svg class="scene incomplete-scene" viewBox="0 0 660 560" preserveAspectRatio="xMidYMid meet" role="img" aria-label="مشهد واحد بين شخصين وقرار على طاولة ودليل أحمر، لا يظهر إلا عبر ثلاث فتحات منفصلة تكشف أجزاءً من الصورة نفسها">
    ...
    <g filter="url(#inc-sh)"><g mask="url(#inc-mask)">
      ...
      ${person(392, 150, 2, NOIR.lead, { hx: 44, sw: 90 })}
