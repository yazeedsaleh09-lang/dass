...
function loginPage(): ProductPage {
  ...
  const html = session
    ...
    : shell('الحساب', 'ارجع للمجلس', 'دخول تجريبي محلي لا يرسل بيانات إلى خادم.', `<section class="auth-layout"><form id="login-form" class="auth-card panel"><div id="product-notice" role="status" aria-live="polite" hidden></div>${field('email', 'البريد الإلكتروني', 'email', '', 'autocomplete="email" required')}${field('password', 'كلمة المرور', 'password', '', 'data-secret autocomplete="current-password" minlength="8" required')}<div class="demo-credential"><span>حساب العرض الجاهز</span><code dir="ltr">demo@dass.local · demoPass8</code></div><div class="form-options"><label class="check-row"><input name="remember" type="checkbox" checked><span>تذكر الجلسة على هذا الجهاز</span></label><button class="secret-toggle" type="button">إظهار كلمة المرور</button></div><button class="btn primary wide" type="submit">دخول تجريبي</button><a data-link="/forgot-password" class="text-link">نسيت كلمة المرور؟</a></form><aside class="auth-side"><span class="auth-seal">دسّ</span><h2>ما عندك حساب؟</h2><p>أنشئ ملفًا محليًا، أو ادخل كضيف من دون بريد.</p><a class="btn" data-link="/signup">إنشاء حساب تجريبي</a><button id="guest-login" class="btn ghost">المتابعة كضيف</button></aside></section>`);
  return {
    active: 'account', title: 'تسجيل الدخول', description: 'الدخول إلى حساب دسّ التجريبي.', html,
    bind(navigate) {
      bindSecretToggle();
...
function signupPage(): ProductPage {
  return {
    active: 'account', title: 'إنشاء حساب', description: 'إنشاء حساب دسّ تجريبي محلي.',
...
function authStatePage(path: string): ProductPage {
  if (path === '/session-ended') return {
    active: 'account', title: 'انتهت الجلسة', description: 'انتهت جلسة حساب دسّ.',
  ...
  if (path === '/verify-email') return {
    active: 'account', title: 'تأكيد البريد', description: 'حالة تأكيد بريد حساب دسّ.',
  ...
  return {
    active: 'account', title: 'إعادة تعيين كلمة المرور', description: 'إعادة تعيين كلمة مرور دسّ.',
...
function pricingPage(): ProductPage {
  return {
    active: 'pricing', title: 'الأسعار', description: 'باقات دسّ الشفافة من دون أفضلية لعب.',
...
function profilePage(): ProductPage {
  ...
  if (gate) return { active: 'account', title: 'الملف الشخصي', description: 'ملف حساب دسّ.', html: shell('حسابي', 'الملف الشخصي', 'إدارة هويتك داخل دسّ.', gate) };
  ...
  return {
    active: 'account', title: 'الملف الشخصي', description: 'إدارة ملف حساب دسّ.',
...
function inventoryPage(): ProductPage {
  ...
  if (gate) return { active: 'account', title: 'المقتنيات', description: 'مقتنيات حساب دسّ.', html: shell('حسابي', 'المقتنيات', 'العناصر المملوكة والمفعّلة.', gate) };
  const inventory = platform.getInventory();
  const owned = PRODUCTS.filter((product) => inventory.ownedProductIds.includes(product.id));
  return {
    active: 'account', title: 'المقتنيات', description: 'إدارة مقتنيات دسّ.',
...
function settingsPage(): ProductPage {
  const session = platform.getSession(); const gate = requireAccount(session);
  if (gate) return { active: 'account', title: 'الإعدادات', description: 'إعدادات حساب دسّ.', html: shell('حسابي', 'الإعدادات', 'التحكم بالتجربة والخصوصية.', gate) };
  ...
  return {
    active: 'account', title: 'الإعدادات', description: 'إعدادات تجربة وخصوصية دسّ.',
    ...
    bind(navigate) {
      ...
      $('#export-local')?.addEventListener('click', () => {
        ...
        const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([payload], { type: 'application/json' })); link.download = 'dass-demo-data.json'; link.click(); URL.revokeObjectURL(link.href);
      });
...
function historyPage(): ProductPage {
  ...
  return { active: 'account', title: 'سجل المباريات', description: 'سجل مباريات حساب دسّ.', html: shell('حسابي', 'سجل المباريات', 'نتائجك عندما تتوفر مزامنة الحساب.', content) };
}

function matchDetailPage(params: URLSearchParams): ProductPage {
  const session = platform.getSession(); const gate = requireAccount(session);
  if (gate) return { active: 'account', title: 'تفاصيل المباراة', description: 'تفاصيل مباراة دسّ.', html: shell('حسابي', 'تفاصيل المباراة', 'ملخص آمن بعد نهاية اللعبة.', gate) };
...
function achievementsPage(): ProductPage {
  ...
  return { active: 'account', title: 'الإنجازات', description: 'إنجازات دسّ المستقبلية.', html: shell('حسابي', 'الإنجازات', 'إنجازات مرتبطة باللعب الحقيقي فقط.', content) };
}
...
function billingPage(): ProductPage {
  ...
  return {
    active: 'account', title: 'الفوترة', description: 'باقة وفواتير حساب دسّ.', html: shell('حسابي', 'الفوترة', 'الباقة وسجل تجارب الدفع المحلية.', content),
    bind(_navigate, refresh) {
...
function checkoutPage(params: URLSearchParams): ProductPage {
  ...
  let checkoutId = '';
  return {
    active: 'store', title: 'إتمام الطلب', description: 'مراجعة طلب دسّ في وضع العرض.',
...
function supportPage(path = '/support'): ProductPage {
  ...
  return {
    active: 'support', title: 'الدعم', description: 'مركز مساعدة ودعم دسّ.',
...
function invitePage(params: URLSearchParams): ProductPage {
  ...
  return {
    active: 'support', title: 'مشاركة دعوة', description: 'مشاركة رابط وكود غرفة دسّ.',
    ...
    bind() {
      ...
      $('#native-share')?.addEventListener('click', async () => {
        try { if (navigator.share) await navigator.share({ title: 'دعوة دسّ', text: message, url: link }); } catch (error) { if ((error as DOMException).name !== 'AbortError') notice(errorMessage(error), 'error'); }
      });
...
function staticPage(path: string): ProductPage {
  ...
  if (path === '/credits') return { title: 'الاعتمادات', description: 'اعتمادات بناء دسّ.', html: shell('دسّ', 'الاعتمادات', 'المنتج مبني بهوية عربية أصلية وتقنيات ويب مفتوحة.', `<section class="prose-page panel"><h2>التصميم والمنتج</h2><p>هوية دسّ ونظامها البصري ومحتواها العربي جزء من المنتج نفسه.</p><h2>التقنيات</h2><p>TypeScript وesbuild وواجهات الويب القياسية، مع توليد QR محليًا. الخط المستخدم هو Tajawal عبر Google Fonts.</p><h2>المحتوى البصري</h2><p>الرسوم الأساسية وواجهات اللعبة مبنية بالكود. صورة المشاركة الاجتماعية مولدة خصيصًا لهذا المشروع ومراجعة للاستخدام الحالي.</p></section>`) };
  if (path === '/about') return { title: 'عن دسّ', description: 'عن لعبة دسّ ورؤيتها.', html: shell('عن دسّ', 'لعبة مجالس سعودية من أول قرار', 'نبني لحظة اجتماعية حقيقية حول شاشة واحدة، مو عزلة داخل كل جوال.', `<section class="prose-page panel"><h2>الفكرة</h2><p>دسّ لعبة نوايا وقرارات سرية. التلفاز يجمع المشهد، والجوال يحفظ القرار، والكشف يصنع القصة التي يكمل المجلس يتكلم عنها.</p><h2>مبادئنا</h2><p>العربية أصل المنتج، والوضوح أهم من الإلحاح التجاري، واللعبة الأساسية لا تُباع على شكل أفضلية.</p><h2>هذه المرحلة</h2><p>تجربة اللعب الجماعي تعمل، بينما الحساب والمتجر والدفع معروضة الآن من خلال مزود محلي صريح إلى أن تُوصل خدمات الإنتاج.</p></section>`) };
  if (path === '/status') return { title: 'حالة الخدمة', description: 'حالة أنظمة دسّ.', html: shell('الحالة', 'الأنظمة بوضوح', 'تحديث محلي يصف ما هو عامل وما يحتاج مزود إنتاج.', `<section class="status-list panel"><article><i class="ok"></i><div><b>إنشاء الغرف والانضمام</b><span>مسار الإنتاج الحالي — تتم مراقبته باختبارات المستودع.</span></div><strong>متاح</strong></article><article><i class="ok"></i><div><b>موقع دسّ العام</b><span>الصفحات والملفات الثابتة.</span></div><strong>متاح</strong></article><article><i class="demo"></i><div><b>الحسابات والمقتنيات</b><span>مزود عرض محلي، بلا مزامنة أجهزة.</span></div><strong>عرض</strong></article><article><i class="demo"></i><div><b>الدفع والبريد والدعم</b><span>غير موصولة بمزودي إنتاج.</span></div><strong>غير موصول</strong></article></section>`) };
  if (path === '/changelog') return { title: 'سجل التغييرات', description: 'أحدث تغييرات منتج دسّ.', html: shell('التحديثات', 'وش تغيّر؟', 'سجل مختصر لما وصل فعليًا، من غير وعود منجزة وهمية.', `<section class="timeline"><article class="panel"><time>يوليو ٢٠٢٦</time><h2>طبقة المنتج التجاري</h2><ul><li>متجر وباقات وملف وإعدادات متجاوبة.</li><li>مزود عرض محلي صريح للحساب والدفع والدعم.</li><li>حالات فراغ وفشل وإلغاء بلا بيانات مختلقة.</li></ul></article><article class="panel"><time>الإصدار الأساسي</time><h2>تثبيت تدفق المجلس</h2><ul><li>إنشاء الغرفة والانضمام اليدوي والـQR.</li><li>استرجاع المضيف ومنع الانضمام المكرر.</li><li>إعادة اتصال اللاعب.</li></ul></article></section>`) };
