...
var require_umd = __commonJS({
  "node_modules/@colyseus/schema/build/umd/index.js"(exports, module) {
    (function(global, factory) {
      ...
      function utf8Write(view3, str, it) {
        var c = 0;
        for (var i = 0, l = str.length; i < l; i++) {
          c = str.charCodeAt(i);
          if (c < 128) {
            ...
            view3[it.offset] = 192 | c >> 6;
            view3[it.offset + 1] = 128 | c & 63;
            it.offset += 2;
          } else if (c < 55296 || c >= 57344) {
            view3[it.offset] = 224 | c >> 12;
            view3[it.offset + 1] = 128 | c >> 6 & 63;
            view3[it.offset + 2] = 128 | c & 63;
            it.offset += 3;
          } else {
            i++;
            c = 65536 + ((c & 1023) << 10 | str.charCodeAt(i) & 1023);
            view3[it.offset] = 240 | c >> 18;
            view3[it.offset + 1] = 128 | c >> 12 & 63;
            view3[it.offset + 2] = 128 | c >> 6 & 63;
            view3[it.offset + 3] = 128 | c & 63;
            it.offset += 4;
          }
        }
      }
...
Schema instances may only have up to 64 fields.`);
      ...
      class ArraySchema {
         ...
         */
        static [(_a$4 = $encoder, _b$4 = $decoder, $filter)](ref, index, view3) {
          return !view3 || typeof ref[$childType] === "string" || view3.isChangeTreeVisible(ref["tmpItems"][index]?.[$changes]);
        }
        static is(type2) {
          return (
            // type format: ["string"]
      ...
      class MapSchema {
         ...
         * - Then, the encoder iterates over all "owned" properties per instance and encodes them.
         */
        static [(_a$3 = $encoder, _b$3 = $decoder, $filter)](ref, index, view3) {
          return !view3 || typeof ref[$childType] === "string" || view3.isChangeTreeVisible((ref[$getByIndex](index) ?? ref.deletedItems[index])[$changes]);
        }
        static is(type2) {
      ...
      class CollectionSchema {
         ...
         * - Then, the encoder iterates over all "owned" properties per instance and encodes them.
         */
        static [(_a$2 = $encoder, _b$2 = $decoder, $filter)](ref, index, view3) {
          return !view3 || typeof ref[$childType] === "string" || view3.isChangeTreeVisible((ref[$getByIndex](index) ?? ref.deletedItems[index])[$changes]);
        }
        static is(type2) {
      ...
      class SetSchema {
         ...
         * - Then, the encoder iterates over all "owned" properties per instance and encodes them.
         */
        static [(_a$1 = $encoder, _b$1 = $decoder, $filter)](ref, index, view3) {
          return !view3 || typeof ref[$childType] === "string" || view3.visible.has((ref[$getByIndex](index) ?? ref.deletedItems[index])[$changes]);
        }
        static is(type2) {
      ...
      function view2(tag = DEFAULT_VIEW_TAG) {
...
Check @type() annotation`);
      ...
      function schema(fieldsAndMethods, name, inherits = Schema) {
        ...
        for (let fieldName in viewTagFields) {
          view2(viewTagFields[fieldName])(klass.prototype, fieldName);
        }
        if (name) {
      ...
      class Schema {
        ...
        static [$filter](ref, index, view3) {
          ...
          if (view3 === void 0) {
            ...
            return view3.isChangeTreeVisible(ref[$changes]);
          } else {
            const tags = view3.tags?.get(ref[$changes]);
...
`;
      ...
      class Encoder2 {
        ...
        // 8KB
        constructor(state) {
          this.sharedBuffer = Buffer.allocUnsafe(Encoder2.BUFFER_SIZE);
        ...
        encode(it = { offset: 0 }, view3, buffer = this.sharedBuffer, changeSetName = "changes", isEncodeAll = changeSetName === "allChanges", initialOffset = it.offset) {
          const hasView = view3 !== void 0;
          const rootChangeTree = this.state[$changes];
          let current = this.root[changeSetName];
          while (current = current.next) {
            const changeTree = current.changeTree;
            if (hasView) {
              if (!view3.isChangeTreeVisible(changeTree)) {
                view3.invisible.add(changeTree);
                continue;
              }
              view3.invisible.delete(changeTree);
            }
            ...
            for (let j = 0; j < numChanges; j++) {
              ...
              if (fieldIndex === void 0 || operation === void 0 || filter && !filter(ref, fieldIndex, view3)) {
                continue;
              }
...
`);
            ...
            return this.encode({ offset: initialOffset }, view3, buffer, changeSetName, isEncodeAll);
          } else {
        ...
        encodeAllView(view3, sharedOffset, it, bytes = this.sharedBuffer) {
          const viewOffset = it.offset;
          this.encode(it, view3, bytes, "allFilteredChanges", true, viewOffset);
          return Buffer.concat([
            bytes.subarray(0, sharedOffset),
            bytes.subarray(viewOffset, it.offset)
          ]);
        }
        encodeView(view3, sharedOffset, it, bytes = this.sharedBuffer) {
          const viewOffset = it.offset;
          for (const [refId, changes] of view3.changes) {
            const changeTree = this.root.changeTrees[refId];
            if (changeTree === void 0) {
              view3.changes.delete(refId);
              continue;
            }
          ...
          view3.changes.clear();
          this.encode(it, view3, bytes, "filteredChanges", false, viewOffset);
          return Buffer.concat([
            bytes.subarray(0, sharedOffset),
            bytes.subarray(viewOffset, it.offset)
          ]);
...
var require_SchemaSerializer = __commonJS({
  "node_modules/colyseus.js/build/cjs/serializer/SchemaSerializer.js"(exports) {
    "use strict";
    var schema = require_umd();
    function getStateCallbacks(room2) {
      try {
        return schema.getDecoderStateCallbacks(room2["serializer"].decoder);
      } catch (e) {
        return void 0;
      }
    }
...
var require_Room = __commonJS({
  "node_modules/colyseus.js/build/cjs/Room.js"(exports) {
    ...
    var Room = class _Room {
      ...
      connect(endpoint, devModeCloseCallback, room2 = this, options, headers) {
        const connection = new Connection.Connection(options.protocol);
        room2.connection = connection;
        connection.events.onmessage = _Room.prototype.onMessageCallback.bind(room2);
        connection.events.onclose = function(e) {
          ...
          if (e.code === Errors.CloseCode.DEVMODE_RESTART && devModeCloseCallback) {
            ...
            room2.onLeave.invoke(e.code, e.reason);
            room2.destroy();
          }
        };
  ...
  "node_modules/colyseus.js/build/cjs/Client.js"(exports) {
    ...
    var Client2 = class {
      ...
      consumeSeatReservation(response, rootSchema, reuseRoomInstance) {
        return tslib.__awaiter(this, void 0, void 0, function* () {
          const room2 = this.createRoom(response.room.name, rootSchema);
          ...
          const targetRoom = reuseRoomInstance || room2;
          room2.connect(this.buildEndpoint(response.room, options, response.protocol), response.devMode && (() => tslib.__awaiter(this, void 0, void 0, function* () {
            console.info(`[Colyseus devMode]: ${String.fromCodePoint(128260)} Re-establishing connection with room id '${room2.roomId}'...`);
            let retryCount = 0;
            ...
            const retryReconnection = () => tslib.__awaiter(this, void 0, void 0, function* () {
              ...
              try {
                ...
                console.info(`[Colyseus devMode]: ${String.fromCodePoint(9989)} Successfully re-established connection with room '${room2.roomId}'`);
              } catch (e) {
      ...
      buildEndpoint(room2, options = {}, protocol = "ws") {
        ...
        if (room2.publicAddress) {
          endpoint += `${room2.publicAddress}`;
        } else {
          endpoint += `${this.settings.hostname}${this.getEndpointPort()}${this.settings.pathname}`;
        }
        const endpointURL = `${endpoint}/${room2.processId}/${room2.roomId}?${searchParams}`;
...
// apps/v2web/src/main.ts
var import_colyseus = __toESM(require_cjs(), 1);

// packages/i18n/src/en.json
var en_default = {
  "ui.appTitle": "Crisis",
  "ui.tagline": "needs 4+ players",
  "ui.language": "Language",
  "ui.hostRoom": "Host a room",
  "ui.yourName": "Your name",
  "ui.createRoom": "Create room",
  "ui.joinTitle": "Join a room",
  "ui.roomCode": "Room code",
  "ui.join": "Join",
  "ui.enterCode": "Enter a room code",
  "ui.connError": "Could not connect. {msg}",
  "ui.lobby": "Lobby — {count} player(s)",
  "ui.shareHint": "Share the room code below. You need 4 players ready to start.",
  "ui.codeLabel": "Room code:",
  "ui.ready": "Ready",
  "ui.unready": "Not ready",
  "ui.start": "Start ({ready}/4)",
  "ui.leave": "Leave",
  "ui.host": "host",
  "ui.you": "you",
  "ui.readyTag": "ready",
  "ui.waitingTag": "waiting",
  "ui.offline": "offline",
  "ui.crisisHeader": "Crisis {index} of {total} · {phase}",
  "ui.crisisHeaderFinal": "Crisis {index} of {total} — FINAL · {phase}",
  "ui.timeLeft": "{s}s left",
  "ui.yourGoal": "Your private goal:",
  "ui.yourInfo": "Your private info:",
  "ui.noInfo": "You hold no private info this crisis.",
  "ui.reveal": "Reveal to group",
  "ui.revealed": "Revealed to the group:",
  "ui.castVote": "Cast your vote:",
  "ui.votedHint": "You voted — you can change it until the timer ends.",
  "ui.votedCount": "{n} of {total} voted",
  "ui.briefingHint": "Read your info. Discussion opens next.",
  "ui.readySkip": "Ready (skip the wait)",
  "ui.discuss": "Discuss it out",
  "ui.continue": "Continue",
  "ui.talk": "Talk",
  "ui.send": "Send",
  "ui.saySomething": "say something…",
  "ui.endingHeader": "Ending: {kind}",
  "ui.theStory": "The story",
  "ui.privateGoals": "Private goals",
  "ui.backHome": "Back to home",
  "ui.votedMark": "voted",
  "signal.agree": "agree",
  "signal.disagree": "disagree",
  "signal.warning": "warning",
  "signal.need_info": "need info",
  "ui.signalMsg": "— {name} signals: {kind}",
  "phase.briefing": "briefing",
  "phase.deliberation": "deliberation",
  "phase.decision": "decision",
  "phase.resolution": "resolution",
  "phase.interlude": "interlude",
  "meter.stability": "Stability",
  "meter.resources": "Resources",
  "meter.cohesion": "Cohesion",
  "reliability.confirmed": "confirmed",
  "reliability.reported": "reported",
  "reliability.contested": "contested",
  "ending.label.success": "Success",
  "ending.label.stable": "Stable",
  "ending.label.fractured": "Fractured",
  "ending.label.collapse": "Collapse",
  "ending.success.text": "More than survival — you kept who you were.",
  "ending.stable.text": "You held. Scarred, tired, but standing.",
  "ending.fractured.text": "You survived — barely, and not whole.",
  "ending.collapse.text": "The settlement does not survive.",
  "goal.outcome.met": "MET",
  "goal.outcome.partly": "PARTLY",
  "goal.outcome.unmet": "UNMET",
  "goal.provider.title": "The Provider",
  "goal.provider.desc": "You want Resources to be the highest meter at the end.",
  "goal.peacemaker.title": "The Peacemaker",
  "goal.peacemaker.desc": "You want Cohesion to be the highest meter at the end.",
  "goal.guardian.title": "The Guardian",
  "goal.guardian.desc": "You want Stability to be the highest meter at the end.",
  "goal.humanitarian.title": "The Humanitarian",
  "goal.humanitarian.desc": "You never want the group to sacrifice people.",
  "goal.hardliner.title": "The Hardliner",
  "goal.hardliner.desc": "Caution is slow death — take the bold option at least twice.",
  "goal.pragmatist.title": "The Pragmatist",
  "goal.pragmatist.desc": "You just want to survive with no meter ever hitting critical.",
  "goal.restless.title": "The Restless",
  "goal.restless.desc": "The status quo bores you — take a real risk at least once.",
  "goal.skeptic.title": "The Skeptic",
  "goal.skeptic.desc": "You distrust rumor — refuse to act on unverified info at least once.",
  "goal.note.highestMet": "{meterKey} ended highest.",
  "goal.note.highestTied": "{meterKey} tied for highest.",
  "goal.note.highestUnmet": "{meterKey} was not the highest meter.",
  "goal.note.unresolved": "Unresolved goal.",
  "goal.note.surviveMet": "Survived with no meter ever critical.",
  "goal.note.survivePartly": "Survived, but a meter went critical along the way.",
  "goal.note.boldMet": "Chose the bold path {bold} time(s).",
  "goal.note.boldPartly": "Chose bold {bold} time(s); needed {need}.",
  "goal.note.boldUnmet": "Only {bold} bold choice(s).",
  "goal.note.leastSafeMet": "The group took a real risk at least once.",
  "goal.note.leastSafeUnmet": "The group always played it safe.",
  "goal.note.neverSacrificeMet": "No one was sacrificed.",
  "goal.note.neverSacrificeUnmet": "The group sacrificed people at least once.",
  "goal.note.refusedMet": "The group refused to act on rumor.",
  "goal.note.refusedPartly": "The group refused rumor once but acted on it elsewhere.",
  "goal.note.refusedUnmet": "The group acted on unverified rumor.",
  "goal.note.collapseVoid": "The settlement fell; nothing else counts.",
  "crisis.water.title": "The Water Ration",
  "crisis.water.brief": "The main cistern is contaminated. Clean water runs out in days. We have to decide how to handle it — now.",
  "crisis.water.card.a": "The engineers can repair the filtration unit in about three days if it gets priority power.",
  "crisis.water.card.b": "A scout says there is a spring two hours out — but the route crosses the old quarantine zone.",
  "crisis.water.card.c": "Someone insists the panic is overblown — the water is fine if you just boil it.",
  "crisis.water.card.d": "Rationing will hit the outer households hardest, and they will resent it.",
  "crisis.water.o1.title": "Divert power to repair the filtration unit",
  "crisis.water.o1.result": "Crews reroute power to the filters. Taps run low for now, but a real fix is coming.",
  "crisis.water.o2.title": "Send a party to the spring",
  "crisis.water.o2.result": "The party reaches the spring — a trickle, not a river, and two return limping.",
  "crisis.water.o3.title": "Strict equal rationing now",
  "crisis.water.o3.result": "Everyone tightens their belt. The outer households feel singled out.",
  "crisis.water.o4.title": "Trust “just boil it” — no rationing",
  "crisis.water.o4.result": "Relief — no rationing. People boil their water and move on.",
  "crisis.newcomers.title": "The Newcomers",
  "crisis.newcomers.brief": "Six outsiders are at the gate — cold, hungry, maybe useful, maybe a burden, maybe a threat. Do we let them in?",
  "crisis.newcomers.card.a": "One of them is a trained medic.",
  "crisis.newcomers.card.b": "A guard swears one matches the description of raiders who hit a settlement to the north.",
  "crisis.newcomers.card.c": "We honestly do not have surplus food for six more mouths this month.",
  "crisis.newcomers.card.d": "You have heard half the settlement already wants them turned away.",
  "crisis.newcomers.o1.title": "Take them all in",
  "crisis.newcomers.o1.result": "You open the gate. The medic quietly gets to work; a few grumble about the extra mouths.",
  "crisis.newcomers.o2.title": "Turn them away",
  "crisis.newcomers.o2.result": "The gate stays shut. You tell yourselves it was necessary.",
  "crisis.newcomers.o3.title": "Take some, refuse the rest",
  "crisis.newcomers.o3.result": "You wave some in and turn others back. No one thinks it was fair.",
  "crisis.newcomers.o4.title": "Detain and interrogate first",
  "crisis.newcomers.o4.result": "You hold them under guard. They are terrified, not raiders. Word spreads that the council jails the desperate.",
  "crisis.sickness.title": "The Sickness",
  "crisis.sickness.brief": "People are falling ill — fever, spreading fast. We have medicine for maybe half the sick. What do we do?",
  "crisis.sickness.link.contamination": "The water was never safe — the fever started at the cistern.",
  "crisis.sickness.link.filtration": "Clean water at least slows the spread.",
  "crisis.sickness.card.a": "Quarantining the sick will probably contain it.",
  "crisis.sickness.card.b": "There is a rumor the sickness came in with the newcomers.",
  "crisis.sickness.card.c": "We can treat about half the sick, no more.",
  "crisis.sickness.o1.title": "Quarantine hard",
  "crisis.sickness.o1.result": "Doors are sealed, the sick behind them. It works — and it costs you something to watch.",
  "crisis.sickness.o2.title": "Treat the vulnerable first, share widely",
  "crisis.sickness.o2.result": "You spread the medicine thin and fair. Some who might have been saved are not; but no one was abandoned.",
  "crisis.sickness.o3.title": "Treat the useful first to keep the settlement running",
  "crisis.sickness.o3.result": "You keep the settlement running by choosing who matters. It survives. So does the memory.",
  "crisis.sickness.o4.title": "Let the medic run structured triage",
  "crisis.sickness.o4.result": "The medic organizes triage with a steady hand — the closest thing to a good outcome anyone could manage.",
  "crisis.breach.title": "The Breach",
  "crisis.breach.brief": "A storm surge is coming and the north wall is failing. This is the one that decides everything. Choose.",
  "crisis.breach.card.a": "Everyone can feel it coming. There is no time to argue long.",
  "crisis.breach.o1.title": "Everyone to the wall — hold it together",
  "crisis.breach.o1.result": "The settlement pulls together and holds the line.",
  "crisis.breach.o1.fail": "People will not pull together; the wall gives way.",
  "crisis.breach.o2.title": "Spend the reserves to reinforce",
  "crisis.breach.o2.result": "The wall is shored up in time.",
  "crisis.breach.o2.fail": "There is nothing left to spend; the reinforcement is half-finished.",
  "crisis.breach.o3.title": "Evacuate to high ground — abandon the stores to save people",
  "crisis.breach.o3.result": "People live; the built place is lost to the water.",
  "log.prologue": "Prologue: the settlement stands. Stability {stability}, Resources {resources}, Cohesion {cohesion}.",
  "log.plain": "{textKey}",
  "log.crisisIntro": "Crisis {index} — {titleKey}: {briefKey}",
  "log.crisisIntroFinal": "Crisis {index} (FINAL) — {titleKey}: {briefKey}",
  "log.infoTrue": "· “{contentKey}” turned out TRUE.",
  "log.infoFalse": "· “{contentKey}” turned out FALSE.",
  "log.chose": "→ The group chose “{titleKey}”. {resultKey}",
  "log.meters": "Meters — Stability {stability}, Resources {resources}, Cohesion {cohesion}.",
  "log.collapse": "COLLAPSE — {meterKey} reached zero. The settlement does not survive.",
  "log.ending": "Ending — {kindKey}: {textKey}"
};

// packages/i18n/src/ar.json
var ar_default = {
  "ui.appTitle": "أزمة",
  "ui.tagline": "يحتاج إلى 4 لاعبين أو أكثر",
  "ui.language": "اللغة",
  "ui.hostRoom": "أنشئ غرفة",
  "ui.yourName": "اسمك",
  "ui.createRoom": "إنشاء غرفة",
  "ui.joinTitle": "انضمّ إلى غرفة",
  "ui.roomCode": "رمز الغرفة",
  "ui.join": "انضمام",
  "ui.enterCode": "أدخل رمز الغرفة",
  "ui.connError": "تعذّر الاتصال. {msg}",
  "ui.lobby": "الردهة — {count} لاعب",
  "ui.shareHint": "شارك رمز الغرفة أدناه. تحتاج إلى استعداد 4 لاعبين للبدء.",
  "ui.codeLabel": "رمز الغرفة:",
  "ui.ready": "مستعد",
  "ui.unready": "غير مستعد",
  "ui.start": "ابدأ ({ready}/4)",
  "ui.leave": "مغادرة",
  "ui.host": "المضيف",
  "ui.you": "أنت",
  "ui.readyTag": "مستعد",
  "ui.waitingTag": "بالانتظار",
  "ui.offline": "غير متصل",
  "ui.crisisHeader": "الأزمة {index} من {total} · {phase}",
  "ui.crisisHeaderFinal": "الأزمة {index} من {total} — النهائية · {phase}",
  "ui.timeLeft": "بقي {s} ث",
  "ui.yourGoal": "هدفك الخاص:",
  "ui.yourInfo": "معلوماتك الخاصة:",
  "ui.noInfo": "لا تملك معلومات خاصة في هذه الأزمة.",
  "ui.reveal": "أظهرها للمجموعة",
  "ui.revealed": "ما أُظهر للمجموعة:",
  "ui.castVote": "أدلِ بصوتك:",
  "ui.votedHint": "لقد صوّتّ — يمكنك تغيير اختيارك حتى انتهاء المؤقّت.",
  "ui.votedCount": "صوّت {n} من {total}",
  "ui.briefingHint": "اقرأ معلوماتك. سيبدأ النقاش بعد قليل.",
  "ui.readySkip": "مستعد (تخطَّ الانتظار)",
  "ui.discuss": "ناقشوا الأمر",
  "ui.continue": "متابعة",
  "ui.talk": "النقاش",
  "ui.send": "إرسال",
  "ui.saySomething": "قل شيئًا…",
  "ui.endingHeader": "النهاية: {kind}",
  "ui.theStory": "الحكاية",
  "ui.privateGoals": "الأهداف الخاصة",
  "ui.backHome": "العودة إلى البداية",
  "ui.votedMark": "صوّت",
  "signal.agree": "أوافق",
  "signal.disagree": "أعترض",
  "signal.warning": "تحذير",
  "signal.need_info": "أحتاج معلومات",
  "ui.signalMsg": "— {name} يشير: {kind}",
  "phase.briefing": "الإحاطة",
  "phase.deliberation": "النقاش",
  "phase.decision": "القرار",
  "phase.resolution": "النتيجة",
  "phase.interlude": "استراحة",
  "meter.stability": "الاستقرار",
  "meter.resources": "الموارد",
  "meter.cohesion": "التماسك",
  "reliability.confirmed": "مؤكَّد",
  "reliability.reported": "مُبلَّغ",
  "reliability.contested": "متنازَع عليه",
  "ending.label.success": "نجاح",
  "ending.label.stable": "مستقر",
  "ending.label.fractured": "منقسم",
  "ending.label.collapse": "انهيار",
  "ending.success.text": "أكثر من مجرّد نجاة — حافظتم على هُويّتكم.",
  "ending.stable.text": "صمدتم. مُنهَكين ومثقلين بالندوب، لكنكم واقفون.",
  "ending.fractured.text": "نجوتم — بالكاد، ولستم كما كنتم.",
  "ending.collapse.text": "لم تنجُ المستوطنة.",
  "goal.outcome.met": "تحقّق",
  "goal.outcome.partly": "جزئيًا",
  "goal.outcome.unmet": "لم يتحقّق",
  "goal.provider.title": "المُعيل",
  "goal.provider.desc": "تريد أن تكون الموارد أعلى مؤشّر في النهاية.",
  "goal.peacemaker.title": "صانع السلام",
  "goal.peacemaker.desc": "تريد أن يكون التماسك أعلى مؤشّر في النهاية.",
  "goal.guardian.title": "الحارس",
  "goal.guardian.desc": "تريد أن يكون الاستقرار أعلى مؤشّر في النهاية.",
  "goal.humanitarian.title": "الإنساني",
  "goal.humanitarian.desc": "لا تريد للمجموعة أن تُضحّي بالناس أبدًا.",
  "goal.hardliner.title": "المتشدّد",
  "goal.hardliner.desc": "الحذر موتٌ بطيء — اختر الخيار الجريء مرّتين على الأقل.",
  "goal.pragmatist.title": "الواقعي",
  "goal.pragmatist.desc": "تريد النجاة فحسب دون أن يبلغ أيّ مؤشّر المستوى الحرج.",
  "goal.restless.title": "المتمرّد",
  "goal.restless.desc": "الوضع الراهن يُملّك — خُض مخاطرة حقيقية مرّة واحدة على الأقل.",
  "goal.skeptic.title": "المرتاب",
  "goal.skeptic.desc": "لا تثق بالشائعات — ارفض التصرّف بناءً على معلومة غير مؤكَّدة مرّة واحدة على الأقل.",
  "goal.note.highestMet": "انتهى {meterKey} كأعلى مؤشّر.",
  "goal.note.highestTied": "تعادل {meterKey} على أعلى مؤشّر.",
  "goal.note.highestUnmet": "لم يكن {meterKey} أعلى مؤشّر.",
  "goal.note.unresolved": "هدف غير محدَّد.",
  "goal.note.surviveMet": "نجاةٌ دون أن يبلغ أيّ مؤشّر المستوى الحرج.",
  "goal.note.survivePartly": "نجاة، لكنّ أحد المؤشّرات بلغ المستوى الحرج في الطريق.",
  "goal.note.boldMet": "اختيار الطريق الجريء {bold} مرّة.",
  "goal.note.boldPartly": "اختيار الجريء {bold} مرّة؛ المطلوب {need}.",
  "goal.note.boldUnmet": "{bold} خيار جريء فقط.",
  "goal.note.leastSafeMet": "خاضت المجموعة مخاطرة حقيقية مرّة على الأقل.",
  "goal.note.leastSafeUnmet": "لعبت المجموعة بأمان دائمًا.",
  "goal.note.neverSacrificeMet": "لم يُضحَّ بأحد.",
  "goal.note.neverSacrificeUnmet": "ضحّت المجموعة بالناس مرّة على الأقل.",
  "goal.note.refusedMet": "رفضت المجموعة التصرّف بناءً على شائعة.",
  "goal.note.refusedPartly": "رفضت المجموعة شائعة مرّة لكنها تصرّفت بناءً على أخرى.",
  "goal.note.refusedUnmet": "تصرّفت المجموعة بناءً على شائعة غير مؤكَّدة.",
  "goal.note.collapseVoid": "سقطت المستوطنة؛ ولا شيء آخر يهمّ.",
  "crisis.water.title": "حصص الماء",
  "crisis.water.brief": "تلوّث الخزّان الرئيسي. سينفد الماء النظيف خلال أيام. علينا أن نقرّر كيف نتصرّف — الآن.",
  "crisis.water.card.a": "يستطيع المهندسون إصلاح وحدة الترشيح خلال ثلاثة أيام تقريبًا إذا مُنحت أولوية الطاقة.",
  "crisis.water.card.b": "يقول كشّاف إنّ هناك نبعًا على بُعد ساعتين — لكنّ الطريق يعبر منطقة الحجر القديمة.",
  "crisis.water.card.c": "يصرّ أحدهم أنّ الذعر مبالَغ فيه — الماء سليم إن غليتَه فحسب.",
  "crisis.water.card.d": "سيطال التقنين البيوت الطرفية أكثر من غيرها، وسيستاؤون من ذلك.",
  "crisis.water.o1.title": "حوّل الطاقة لإصلاح وحدة الترشيح",
  "crisis.water.o1.result": "يعيد العمّال توجيه الطاقة إلى المرشّحات. تشحّ الصنابير مؤقتًا، لكنّ حلًّا حقيقيًا في الطريق.",
  "crisis.water.o2.title": "أرسل فريقًا إلى النبع",
  "crisis.water.o2.result": "يصل الفريق إلى النبع — خيط ماء لا نهر، ويعود اثنان يعرجان.",
  "crisis.water.o3.title": "تقنين صارم ومتساوٍ الآن",
  "crisis.water.o3.result": "يشدّ الجميع أحزمتهم. تشعر البيوت الطرفية أنها استُفردت.",
  "crisis.water.o4.title": "ثِق بنصيحة «اغلِه فحسب» — بلا تقنين",
  "crisis.water.o4.result": "ارتياح — بلا تقنين. يغلي الناس ماءهم ويمضون.",
  "crisis.newcomers.title": "القادمون الجدد",
  "crisis.newcomers.brief": "ستة غرباء عند البوابة — بردٌ وجوع، ربما نافعون، وربما عبء، وربما خطر. أندخلهم؟",
  "crisis.newcomers.card.a": "أحدهم مسعف مدرَّب.",
  "crisis.newcomers.card.b": "يُقسم حارس أنّ أحدهم يطابق أوصاف مغيرين هاجموا مستوطنة في الشمال.",
  "crisis.newcomers.card.c": "بصراحة لا يوجد لدينا فائض طعام لستة أفواه إضافية هذا الشهر.",
  "crisis.newcomers.card.d": "سمعتَ أنّ نصف المستوطنة يريد طردهم أصلًا.",
  "crisis.newcomers.o1.title": "أدخلهم جميعًا",
  "crisis.newcomers.o1.result": "تفتح البوابة. يشرع المسعف في العمل بهدوء؛ ويتذمّر قِلّة من الأفواه الزائدة.",
  "crisis.newcomers.o2.title": "اطردهم",
  "crisis.newcomers.o2.result": "تبقى البوابة موصدة. تقنعون أنفسكم أنّ ذلك كان ضروريًا.",
  "crisis.newcomers.o3.title": "أدخل بعضهم وارفض الباقي",
  "crisis.newcomers.o3.result": "تُدخل بعضهم وتردّ آخرين. لا أحد يظنّ أنّ ذلك كان عادلًا.",
  "crisis.newcomers.o4.title": "احتجزهم واستجوبهم أولًا",
  "crisis.newcomers.o4.result": "تحتجزهم تحت الحراسة. إنهم مرعوبون لا مغيرون. ينتشر الخبر أنّ المجلس يسجن اليائسين.",
  "crisis.sickness.title": "الوباء",
  "crisis.sickness.brief": "يمرض الناس — حُمّى تنتشر بسرعة. لدينا دواء لنصف المرضى ربما. ماذا نفعل؟",
  "crisis.sickness.link.contamination": "لم يكن الماء آمنًا قط — بدأت الحمّى من الخزّان.",
  "crisis.sickness.link.filtration": "الماء النظيف يُبطئ الانتشار على الأقل.",
  "crisis.sickness.card.a": "عزل المرضى سيحتوي الوباء على الأرجح.",
  "crisis.sickness.card.b": "تشيع إشاعة أنّ الوباء دخل مع القادمين الجدد.",
  "crisis.sickness.card.c": "نستطيع علاج نصف المرضى تقريبًا، لا أكثر.",
  "crisis.sickness.o1.title": "اعزلهم بصرامة",
  "crisis.sickness.o1.result": "تُغلق الأبواب والمرضى خلفها. ينجح الأمر — ويكلّفكم مشاهدته شيئًا من أنفسكم.",
  "crisis.sickness.o2.title": "عالج الأضعف أولًا ووزّع الدواء على الجميع",
  "crisis.sickness.o2.result": "توزّعون الدواء قليلًا وبعدل. لم يُنقَذ بعض من كان يمكن إنقاذهم؛ لكن لم يُهمَل أحد.",
  "crisis.sickness.o3.title": "عالج الأنفع أولًا لتبقى المستوطنة تعمل",
  "crisis.sickness.o3.result": "تُبقون المستوطنة تعمل باختيار من يهمّ. تنجو. وتبقى الذكرى كذلك.",
  "crisis.sickness.o4.title": "دع المسعف يدير فرزًا منظّمًا",
  "crisis.sickness.o4.result": "ينظّم المسعف الفرز بيدٍ ثابتة — أقرب ما يكون إلى نتيجة جيدة يمكن بلوغها.",
  "crisis.breach.title": "الثغرة",
  "crisis.breach.brief": "عاصفةٌ عارمة قادمة والجدار الشمالي ينهار. هذه هي الأزمة التي تحسم كلّ شيء. اختاروا.",
  "crisis.breach.card.a": "يشعر الجميع باقترابها. لا وقت لنقاشٍ طويل.",
  "crisis.breach.o1.title": "الجميع إلى الجدار — اصمدوا معًا",
  "crisis.breach.o1.result": "تتماسك المستوطنة وتصمد على الخط.",
  "crisis.breach.o1.fail": "لا يتماسك الناس؛ فينهار الجدار.",
  "crisis.breach.o2.title": "أنفق الاحتياطي للتعزيز",
  "crisis.breach.o2.result": "يُدعَّم الجدار في الوقت المناسب.",
  "crisis.breach.o2.fail": "لم يبقَ ما يُنفَق؛ فالتعزيز نصف مكتمل.",
  "crisis.breach.o3.title": "أخْلوا إلى الأرض المرتفعة — اتركوا المؤن لإنقاذ الناس",
  "crisis.breach.o3.result": "ينجو الناس؛ ويبتلع الماء المكان الذي بنوه.",
  "log.prologue": "المقدّمة: المستوطنة صامدة. الاستقرار {stability}، الموارد {resources}، التماسك {cohesion}.",
  "log.plain": "{textKey}",
  "log.crisisIntro": "الأزمة {index} — {titleKey}: {briefKey}",
  "log.crisisIntroFinal": "الأزمة {index} (النهائية) — {titleKey}: {briefKey}",
  "log.infoTrue": "· تبيّن أنّ «{contentKey}» صحيحة.",
  "log.infoFalse": "· تبيّن أنّ «{contentKey}» خاطئة.",
  "log.chose": "← اختارت المجموعة «{titleKey}». {resultKey}",
  "log.meters": "المؤشّرات — الاستقرار {stability}، الموارد {resources}، التماسك {cohesion}.",
  "log.collapse": "انهيار — بلغ {meterKey} الصفر. لم تنجُ المستوطنة.",
  "log.ending": "النهاية — {kindKey}: {textKey}"
};

// packages/i18n/src/v2.en.json
var v2_en_default = {
  "v2.ui.appTitle": "The Council",
  "v2.ui.tagline": "6 people, one city, no easy calls. Talk it out loud.",
  "v2.ui.hostRoom": "Host a session",
  "v2.ui.yourName": "Your name",
  "v2.ui.createRoom": "Create session",
  "v2.ui.joinTitle": "Join a session",
  "v2.ui.roomCode": "Code",
  "v2.ui.join": "Join",
  "v2.ui.enterCode": "Enter a code",
  "v2.ui.connError": "Couldn't connect. {msg}",
  "v2.ui.language": "Language",
  "v2.ui.lobby": "The council is gathering — {count} seated",
  "v2.ui.shareHint": "Share the code. 4 seats minimum to begin.",
  "v2.ui.ready": "I'm seated",
  "v2.ui.unready": "Stand up",
  "v2.ui.start": "Begin ({ready}/4)",
  "v2.ui.leave": "Leave",
  "v2.ui.yourRole": "You are",
  "v2.ui.yourInfo": "What only you know",
  "v2.ui.revealToGroup": "Say this out loud (share)",
  "v2.ui.revealed": "Put on the table",
  "v2.ui.discussPrompt": "Talk. Look up. Decide together.",
  "v2.ui.commitPrompt": "Where do you stand? Commit in secret.",
  "v2.ui.commitHint": "Everyone reveals at once. No peeking.",
  "v2.ui.undecided": "Undecided",
  "v2.ui.committed": "Locked in",
  "v2.ui.tapToCommit": "tap your lean",
  "v2.ui.whereTheyStand": "Where everyone stood",
  "v2.ui.challengePrompt": "Surprised by someone? Ask them — out loud. You can still change your mind.",
  "v2.ui.lockingIn": "Locking in…",
  "v2.ui.cityStatus": "The city",
  "v2.ui.theStory": "How it went",
  "v2.ui.roleOutcomes": "What it cost each of you",
  "v2.ui.backHome": "Back",
  "v2.ui.next": "Continue",
  "v2.ui.waiting": "waiting on the room…",
  "v2.ui.host": "host",
  "v2.ui.you": "you",
  "v2.ui.seated": "seated",
  "v2.phase.briefing": "the situation",
  "v2.phase.discussion": "talk it out",
  "v2.phase.commit": "commit",
  "v2.phase.reveal": "reveal",
  "v2.phase.challenge": "last word",
  "v2.phase.resolution": "the fallout",
  "v2.phase.interlude": "breathe",
  "v2.var.infrastructure": "Infrastructure",
  "v2.var.trust": "Public trust",
  "v2.var.economy": "Economy",
  "v2.var.health": "Public health",
  "v2.band.stable": "steady",
  "v2.band.strained": "strained",
  "v2.band.critical": "critical",
  "v2.conf.confirmed": "confirmed",
  "v2.conf.rumored": "unverified",
  "v2.role.hospital.name": "Hospital Director",
  "v2.role.hospital.blurb": "The wards are your people. Keep them alive.",
  "v2.role.infrastructure.name": "Infrastructure Chief",
  "v2.role.infrastructure.blurb": "If the lights and water fail, nothing else matters.",
  "v2.role.economy.name": "Economic Steward",
  "v2.role.economy.blurb": "A city that goes broke can't save anyone.",
  "v2.role.trust.name": "Public Trust Liaison",
  "v2.role.trust.blurb": "Lose the people's faith and you've lost the city.",
  "v2.role.emergency.name": "Emergency Commander",
  "v2.role.emergency.blurb": "When it breaks, your crews are the ones who go in.",
  "v2.role.community.name": "Community Representative",
  "v2.role.community.blurb": "You speak for the street, not the office.",
  "v2.ending.label.thriving": "The city thrived",
  "v2.ending.label.holding": "The city held",
  "v2.ending.label.fractured": "The city fractured",
  "v2.ending.label.collapse": "The city fell",
  "v2.ending.thriving.text": "You didn't just survive it. You kept the city whole.",
  "v2.ending.holding.text": "Scarred and tired — but still standing.",
  "v2.ending.fractured.text": "You made it through. Something didn't.",
  "v2.ending.collapse.text": "It came apart in your hands.",
  "v2.outcome.stable": "{role} — your part of the city held.",
  "v2.outcome.strained": "{role} — your part is strained, but standing.",
  "v2.outcome.critical": "{role} — your part paid the price.",
  "v2.cred.true": "You raised a warning nobody would confirm. It was true, and it came too late to matter.",
  "v2.cred.false": "You put an unverified word on the table. It turned out wrong.",
  "v2.water.title": "The water",
  "v2.water.brief": "The main reservoir's gone bad. Three days of clean water left, maybe less. We decide now.",
  "v2.water.info.hospital": "The east ward is more exposed than anyone's saying — the sick and old are there.",
  "v2.water.info.economy": "A full shutdown will cost far more than the officials are admitting.",
  "v2.water.info.infra": "Engineers can fix the filtration in three days — if they get priority power.",
  "v2.water.o_fix.title": "Fix filtration first",
  "v2.water.o_fix.result": "Crews reroute power to the filters. Taps run thin, but a real fix is coming.",
  "v2.water.o_ration.title": "Ration hard, right now",
  "v2.water.o_ration.result": "Everyone tightens their belt. The outer streets feel singled out.",
  "v2.water.o_wait.title": "Tell people to boil it",
  "v2.water.o_wait.result": "No rationing, no panic. People boil their water and move on.",
  "v2.power.title": "The grid",
  "v2.power.brief": "A substation's failing. There isn't enough power for everyone at once. Who gets it?",
  "v2.power.callout.contamination": "The water was never really safe — now the fever's spreading, and it needs power to fight.",
  "v2.power.callout.fixed": "The filtration work paid off — clean water is holding the line.",
  "v2.power.info.infra": "If we push the grid past this, we risk a cascade that takes days to undo.",
  "v2.power.info.emergency": "My crews say the industrial district could go dark for a week and still recover.",
  "v2.power.o_hospital.title": "Power to the hospitals",
  "v2.power.o_hospital.result": "The wards stay lit. Whole sectors go dark, and they notice.",
  "v2.power.o_spread.title": "Spread it thin, fairly",
  "v2.power.o_spread.result": "Nobody's fully covered, but nobody's abandoned either.",
  "v2.power.o_industry.title": "Keep industry running",
  "v2.power.o_industry.result": "The money keeps flowing. The clinics run on generators and prayers.",
  "v2.unrest.title": "The square",
  "v2.unrest.brief": "People are in the streets. Angry, loud, and not leaving. This tips one way or the other tonight.",
  "v2.unrest.callout.resentment": "The rationing never sat right with the outer streets. This is where it comes back.",
  "v2.unrest.callout.dark": "The sectors you left in the dark are the ones out here now.",
  "v2.unrest.info.trust": "They'll disperse if one of us goes out and actually listens. Send the wrong face and it explodes.",
  "v2.unrest.info.community": "I hear there's someone stirring this on purpose — but I can't prove it.",
  "v2.unrest.o_talk.title": "Go out and listen",
  "v2.unrest.o_talk.result": "It takes all night, but the square empties on its own. It costs to make the promises stick.",
  "v2.unrest.o_force.title": "Clear the square",
  "v2.unrest.o_force.result": "It's quiet by morning. It's the kind of quiet that remembers.",
  "v2.unrest.o_ignore.title": "Wait it out",
  "v2.unrest.o_ignore.result": "You hold the line and say nothing. The anger doesn't leave — it settles in.",
  "v2.storm.title": "The storm",
  "v2.storm.brief": "A surge is coming and the north wall is failing. This is the one that decides everything. Choose.",
  "v2.storm.callout.heavy": "The way the square was handled — people remember. Not everyone will pull with you tonight.",
  "v2.storm.info.emergency": "We can hold the wall, or save the stores, or move the people. Not all three.",
  "v2.storm.o_together.title": "Everyone to the wall",
  "v2.storm.o_together.result": "The city pulls together and holds the line — if it still trusts itself enough to.",
  "v2.storm.o_spend.title": "Spend everything to reinforce",
  "v2.storm.o_spend.result": "The wall holds. The treasury doesn't.",
  "v2.storm.o_evacuate.title": "Move the people, lose the ground",
  "v2.storm.o_evacuate.result": "People live. The built city is given to the water.",
  "v2.log.prologue": "The city stands. For now.",
  "v2.log.callout": "{textKey}",
  "v2.log.crisis": "— {index}. {titleKey}: {briefKey}",
  "v2.log.crisisFinal": "— {index}. (the last one) {titleKey}: {briefKey}",
  "v2.log.chose": "The council chose: {titleKey}. {resultKey}",
  "v2.log.rumorFalse": "(Turned out false: {textKey})",
  "v2.log.ending": "{kindKey}. {textKey}"
};

// packages/i18n/src/v2.ar.json
var v2_ar_default = {
  "v2.ui.appTitle": "المجلس",
  "v2.ui.tagline": "٦ أشخاص، مدينة وحدة، وقرارات ما فيها شي سهل. اتكلموا بصوت عالٍ.",
  "v2.ui.hostRoom": "سوِّ جلسة",
  "v2.ui.yourName": "اسمك",
  "v2.ui.createRoom": "إنشاء جلسة",
  "v2.ui.joinTitle": "ادخل جلسة",
  "v2.ui.roomCode": "الرمز",
  "v2.ui.join": "دخول",
  "v2.ui.enterCode": "اكتب الرمز",
  "v2.ui.connError": "ما قدرنا نتصل. {msg}",
  "v2.ui.language": "اللغة",
  "v2.ui.lobby": "المجلس يتجمّع — {count} حاضر",
  "v2.ui.shareHint": "شارك الرمز. لازم ٤ على الأقل عشان نبدأ.",
  "v2.ui.ready": "أنا جاهز",
  "v2.ui.unready": "مو جاهز",
  "v2.ui.start": "نبدأ ({ready}/4)",
  "v2.ui.leave": "خروج",
  "v2.ui.yourRole": "دورك",
  "v2.ui.yourInfo": "اللي تعرفه أنت بس",
  "v2.ui.revealToGroup": "قلها بصوت عالٍ (شاركها)",
  "v2.ui.revealed": "اللي انحط على الطاولة",
  "v2.ui.discussPrompt": "اتكلموا. ارفعوا نظركم. قرّروا مع بعض.",
  "v2.ui.commitPrompt": "وش موقفك؟ اختر بالسر.",
  "v2.ui.commitHint": "الكل يكشف مع بعض. ما فيه تطليع.",
  "v2.ui.undecided": "ما قررت",
  "v2.ui.committed": "مثبّت",
  "v2.ui.tapToCommit": "اضغط ميولك",
  "v2.ui.whereTheyStand": "وين وقف كل واحد",
  "v2.ui.challengePrompt": "استغربت من أحد؟ اسأله — بصوت عالٍ. لسا تقدر تغيّر رأيك.",
  "v2.ui.lockingIn": "نثبّت…",
  "v2.ui.cityStatus": "المدينة",
  "v2.ui.theStory": "كيف صارت",
  "v2.ui.roleOutcomes": "وش كلّف كل واحد فيكم",
  "v2.ui.backHome": "رجوع",
  "v2.ui.next": "التالي",
  "v2.ui.waiting": "ننتظر بقية المجلس…",
  "v2.ui.host": "المضيف",
  "v2.ui.you": "أنت",
  "v2.ui.seated": "حاضر",
  "v2.phase.briefing": "الوضع",
  "v2.phase.discussion": "نقاش",
  "v2.phase.commit": "الموقف",
  "v2.phase.reveal": "الكشف",
  "v2.phase.challenge": "آخر كلمة",
  "v2.phase.resolution": "النتيجة",
  "v2.phase.interlude": "خذوا نفس",
  "v2.var.infrastructure": "البنية التحتية",
  "v2.var.trust": "ثقة الناس",
  "v2.var.economy": "الاقتصاد",
  "v2.var.health": "الصحة العامة",
  "v2.band.stable": "ثابت",
  "v2.band.strained": "متوتّر",
  "v2.band.critical": "حرِج",
  "v2.conf.confirmed": "مؤكّد",
  "v2.conf.rumored": "غير مؤكّد",
  "v2.role.hospital.name": "مدير المستشفى",
  "v2.role.hospital.blurb": "الأجنحة ناسك. خلّهم عايشين.",
  "v2.role.infrastructure.name": "رئيس البنية التحتية",
  "v2.role.infrastructure.blurb": "إذا وقفت الكهرباء والماء، ما بقي شي يهم.",
  "v2.role.economy.name": "المسؤول الاقتصادي",
  "v2.role.economy.blurb": "مدينة تفلس ما تقدر تنقذ أحد.",
  "v2.role.trust.name": "مسؤول ثقة الناس",
  "v2.role.trust.blurb": "تفقد ثقة الناس، تفقد المدينة.",
  "v2.role.emergency.name": "قائد الطوارئ",
  "v2.role.emergency.blurb": "لما ينهار الوضع، فرقك هي اللي تدخل.",
  "v2.role.community.name": "ممثّل الأهالي",
  "v2.role.community.blurb": "تتكلم باسم الشارع، مو المكتب.",
  "v2.ending.label.thriving": "المدينة ازدهرت",
  "v2.ending.label.holding": "المدينة صمدت",
  "v2.ending.label.fractured": "المدينة انقسمت",
  "v2.ending.label.collapse": "المدينة سقطت",
  "v2.ending.thriving.text": "ما نجوتوا بس. حافظتوا على المدينة كاملة.",
  "v2.ending.holding.text": "متعبين وفيكم ندوب — لكنكم واقفين.",
  "v2.ending.fractured.text": "عدّيتوا. لكن شي ما عدّى.",
  "v2.ending.collapse.text": "انهارت بين أيديكم.",
  "v2.outcome.stable": "{role} — قسمك من المدينة صمد.",
  "v2.outcome.strained": "{role} — قسمك متوتّر، لكنه واقف.",
  "v2.outcome.critical": "{role} — قسمك دفع الثمن.",
  "v2.cred.true": "رفعت تحذير ما حد أكّده. طلع صح، وجاء متأخر.",
  "v2.cred.false": "حطيت كلام غير مؤكّد على الطاولة. طلع غلط.",
  "v2.water.title": "الماء",
  "v2.water.brief": "الخزّان الرئيسي خرب. باقي ثلاثة أيام ماء نظيف، يمكن أقل. نقرّر الحين.",
  "v2.water.info.hospital": "الحي الشرقي معرّض أكثر مما يقولون — المرضى وكبار السن هناك.",
  "v2.water.info.economy": "الإغلاق الكامل بيكلّف أكثر بكثير مما يعترف فيه المسؤولين.",
  "v2.water.info.infra": "المهندسين يقدرون يصلّحون الترشيح في ثلاثة أيام — إذا أخذوا أولوية الكهرباء.",
  "v2.water.o_fix.title": "صلّحوا الترشيح أول",
  "v2.water.o_fix.result": "العمّال يحوّلون الكهرباء للمرشّحات. الماء يقل شوي، بس الحل الحقيقي جاي.",
  "v2.water.o_ration.title": "قنّنوا بشدّة، الحين",
  "v2.water.o_ration.result": "الكل يشدّ حزامه. الأحياء الطرفية تحس إنها اتخصّت.",
  "v2.water.o_wait.title": "قولوا للناس يغلونه",
  "v2.water.o_wait.result": "لا تقنين ولا ذعر. الناس يغلون ماءهم ويمشون حالهم.",
  "v2.power.title": "الكهرباء",
  "v2.power.brief": "محطة تحويل تنهار. ما فيه كهرباء تكفي الكل مرّة وحدة. مين ياخذها؟",
  "v2.power.callout.contamination": "الماء ما كان آمن أصلاً — الحين الحمّى تنتشر، وتحتاج كهرباء نصارعها.",
  "v2.power.callout.fixed": "شغل الترشيح أثمر — الماء النظيف ماسك الوضع.",
  "v2.power.info.infra": "إذا ضغطنا الشبكة أكثر من كذا، نخاطر بانهيار متسلسل ياخذ أيام نصلّحه.",
  "v2.power.info.emergency": "فرقي يقولون المنطقة الصناعية تقدر تطفى أسبوع وترجع.",
  "v2.power.o_hospital.title": "الكهرباء للمستشفيات",
  "v2.power.o_hospital.result": "الأجنحة تبقى مضيّة. أحياء كاملة تطفى، ويلاحظون.",
  "v2.power.o_spread.title": "وزّعوها بالعدل",
  "v2.power.o_spread.result": "ما فيه أحد مغطّى كامل، بس ما فيه أحد متروك.",
  "v2.power.o_industry.title": "خلّوا الصناعة تشتغل",
  "v2.power.o_industry.result": "الفلوس تستمر. العيادات تشتغل على مولّدات ودعوات.",
  "v2.unrest.title": "الساحة",
  "v2.unrest.brief": "الناس في الشوارع. غاضبين، عالي صوتهم، وما راحوا. الليلة تميل لجهة أو جهة.",
  "v2.unrest.callout.resentment": "التقنين ما راق للأحياء الطرفية. هنا يرجع لكم.",
  "v2.unrest.callout.dark": "الأحياء اللي تركتوها في الظلام هي اللي برّه الحين.",
  "v2.unrest.info.trust": "بيتفرّقون إذا طلع لهم واحد منا يسمعهم فعلاً. طلّع الوجه الغلط وينفجر الوضع.",
  "v2.unrest.info.community": "أسمع إن فيه أحد يحرّك هذا عمداً — بس ما أقدر أثبته.",
  "v2.unrest.o_talk.title": "اطلعوا واسمعوا",
  "v2.unrest.o_talk.result": "تاخذ الليل كله، بس الساحة تفضى من نفسها. تكلّف عشان الوعود تثبت.",
  "v2.unrest.o_force.title": "فرّقوا الساحة",
  "v2.unrest.o_force.result": "الوضع يهدأ الصبح. هدوء من النوع اللي ما ينسى.",
  "v2.unrest.o_ignore.title": "استنّوا لين يروحون",
  "v2.unrest.o_ignore.result": "تمسكون الوضع وما تقولون شي. الغضب ما يروح — يستقر.",
  "v2.storm.title": "العاصفة",
  "v2.storm.brief": "موجة قادمة والجدار الشمالي ينهار. هذي الأزمة اللي تحسم كل شي. اختاروا.",
  "v2.storm.callout.heavy": "طريقة التعامل مع الساحة — الناس تذكر. مو كلهم بيشدّون معكم الليلة.",
  "v2.storm.info.emergency": "نقدر نمسك الجدار، أو ننقذ المخازن، أو ننقل الناس. مو الثلاثة.",
  "v2.storm.o_together.title": "الكل على الجدار",
  "v2.storm.o_together.result": "المدينة تتماسك وتصمد على الخط — إذا كانت لسا تثق في نفسها كفاية.",
  "v2.storm.o_spend.title": "اصرفوا كل شي للتعزيز",
  "v2.storm.o_spend.result": "الجدار يصمد. الخزينة لا.",
  "v2.storm.o_evacuate.title": "انقلوا الناس، واتركوا الأرض",
  "v2.storm.o_evacuate.result": "الناس تنجو. المدينة المبنية تُسلَّم للماء.",
  "v2.log.prologue": "المدينة صامدة. للحين.",
  "v2.log.callout": "{textKey}",
  "v2.log.crisis": "— {index}. {titleKey}: {briefKey}",
  "v2.log.crisisFinal": "— {index}. (الأخيرة) {titleKey}: {briefKey}",
  "v2.log.chose": "المجلس اختار: {titleKey}. {resultKey}",
  "v2.log.rumorFalse": "(طلع غلط: {textKey})",
  "v2.log.ending": "{kindKey}. {textKey}"
};
...
var DICTS = {
  en: { ...en_default, ...v2_en_default },
  ar: { ...ar_default, ...v2_ar_default }
};
function dir(locale2) {
  return locale2 === "ar" ? "rtl" : "ltr";
}
function detectLocale(pref) {
  const raw = pref ?? (typeof navigator !== "undefined" && navigator.language ? navigator.language : "en") ?? "en";
...
function t(locale2, key, params) {
  const raw = DICTS[locale2]?.[key] ?? DICTS.en[key] ?? key;
  return interpolate(raw, params);
}
function renderLog(entry, locale2) {
  const p = {};
  for (const [k, v] of Object.entries(entry.params ?? {})) {
    p[k] = typeof v === "string" && k.endsWith("Key") ? t(locale2, v) : v;
  }
  return t(locale2, entry.key, p);
}
...
var SERVER = server();
var locale = localStorage.getItem("v2locale") || (detectLocale(qs.get("lang")) === "en" && !qs.get("lang") ? "ar" : detectLocale(qs.get("lang") ?? "ar"));
...
var ltr = (s) => el("span", { className: "ltr", textContent: String(s) });
function applyDir() {
...
function setLocale(l) {
  locale = l;
  localStorage.setItem("v2locale", l);
  applyDir();
  buildLang();
  render();
}
...
async function create(nick) {
  client = new import_colyseus.Client(SERVER);
  room = await client.create("match", { nickname: nick });
  handlers(room);
}
async function join(code, nick) {
  if (!code) throw new Error(TR("v2.ui.enterCode"));
  client = new import_colyseus.Client(SERVER);
  room = await client.joinById(code, { nickname: nick });
  handlers(room);
}
function attempt(p) {
  lastError = "";
  void p.catch((e) => {
    lastError = TR("v2.ui.connError", { msg: e?.message ?? String(e) });
    render();
  });
}
function timeLeft(v) {
  if (!v.phaseEndsAt) return "";
  const s = Math.max(0, Math.round((v.phaseEndsAt - Date.now()) / 1e3));
  return el("span", { className: "timer" + (s <= 8 ? " urgent" : "") }, ltr(`${s}s`));
}
function bandsRow(v) {
  return el("div", { className: "chips" }, ...["infrastructure", "trust", "economy", "health"].map((k) => el("span", { className: "chip " + v.cityBands[k] }, `${TR(`v2.var.${k}`)}: ${TR(`v2.band.${v.cityBands[k]}`)}`)));
}
function optTitle(v, id) {
  const o = v.crisis?.options.find((x) => x.id === id);
  return o ? TR(o.titleKey) : TR("v2.ui.undecided");
}
function renderHome() {
  const nick = el("input", { placeholder: TR("v2.ui.yourName") });
  nick.value = localStorage.getItem("v2nick") ?? "";
  const code = el("input", { placeholder: TR("v2.ui.roomCode") });
  const getNick = () => {
    const n = nick.value.trim() || "ضيف";
    localStorage.setItem("v2nick", n);
    return n;
  };
  app.replaceChildren(
...
function renderLobby(v) {
  ...
  app.replaceChildren(
    ...
    el("div", { className: "row" }, ...v.players.map((p) => el("span", { className: "pill" + (p.id === v.you.id ? " you" : "") }, `${p.nickname}${p.isHost ? " ★" : ""} ${p.ready ? "✓" : "…"}`))),
...
function infoBlock(v, withReveal) {
  if (!v.you.info.length) return null;
  return el(
    "div",
    { className: "card" },
    el("h3", {}, TR("v2.ui.yourInfo")),
    ...v.you.info.map((c) => {
      const shared = v.revealedInfo.some((r) => r.infoId === c.id && r.playerId === v.you.id);
      const line = el(
        "div",
        { style: "margin:8px 0" },
        el("span", { className: "tag" + (c.confidence === "rumored" ? " rumored" : "") }, TR(`v2.conf.${c.confidence}`)),
        " ",
        TR(c.textKey)
      );
...
function tableInfo(v) {
  if (!v.revealedInfo.length) return null;
  return el(
    "div",
    { className: "card" },
    el("h3", {}, TR("v2.ui.revealed")),
    ...v.revealedInfo.map((r) => el(
      "div",
      { style: "margin:6px 0" },
      el("span", { className: "tag" + (r.confidence === "rumored" ? " rumored" : "") }, TR(`v2.conf.${r.confidence}`)),
      " ",
      TR(r.textKey)
    ))
  );
}
function optionButtons(v) {
  return el("div", {}, ...(v.crisis?.options ?? []).map((o) => btn((v.you.commit === o.id ? "✓ " : "") + TR(o.titleKey), () => room?.send("commit", { optionId: o.id }), v.you.commit === o.id ? "sel" : "")));
}
function renderMatch(v) {
  ...
  if (v.phase === "briefing") {
    ...
    kids.push(el("div", { className: "card" }, ...v.players.map((p) => el("div", { className: "reveal-row" }, el("b", {}, p.nickname), el("span", {}, optTitle(v, p.revealedCommit))))));
    ...
    kids.push(el("div", { className: "chips" }, ...(v.crisis?.options ?? []).map((o) => el("span", { className: "chip" }, `${TR(o.titleKey)}: `, ltr(tally[o.id] ?? 0)))));
    kids.push(el("h3", {}, TR("v2.ui.tapToCommit")));
    kids.push(optionButtons(v));
    kids.push(timeLeft(v));
  ...
  kids.push(el("div", { className: "row", style: "margin-top:14px" }, ...v.players.map((p) => el(
    "span",
    { className: "pill" + (p.id === v.you.id ? " you" : "") },
    `${p.nickname}`,
    p.nameKey ? el("span", { className: "muted" }, ` · ${TR(p.nameKey)}`) : "",
